...
public class CycleUndirectedDFS {
    ...
    public static boolean dfsTraverser(ArrayList<Edge>[] graph, Boolean[] isVisisted,
            Integer vertex, int parent) {
        isVisisted[vertex] = true;

        for (int i = 0; i < graph[vertex].size(); i++) {
            Integer currVertex = graph[vertex].get(i).destination;
            if (!isVisisted[currVertex]) {
                if (dfsTraverser(graph, isVisisted, currVertex, vertex)) {
                    return true;
                }
            } else if (currVertex != parent) {
                return true;
            }
        }
        return false;
    ...
    public static void dfsTraverserUtil(ArrayList<Edge>[] graph, Boolean[] isVisisted) {
        for (int i = 0; i < graph.length; i++) {
            if (!isVisisted[i]) {
                if (dfsTraverser(graph, isVisisted, i, -1)) {
